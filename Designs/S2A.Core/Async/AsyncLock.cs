...
namespace S2A.Core
{
    ...
    public static class AsyncLock
    {
        ...
        /// </summary>
        private static Dictionary<string, SemaphoreDetails> _semaphores = new Dictionary<string, SemaphoreDetails>();
        ...
        public static async Task<T> LockResultAsync<T>(string key, Func<Task<T>> task, int maxAccessCount = 1)
        {
            ...
            try
            {
                ...
                if (!_semaphores.ContainsKey(key))
                    // Create it
                    _semaphores.Add(key, new SemaphoreDetails(key, maxAccessCount));
            }
            finally
            {
                //
